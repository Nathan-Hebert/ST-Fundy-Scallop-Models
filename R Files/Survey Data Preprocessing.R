...
# Intersect each respective year-month of MWSH and SHF data with same year-month 
# of bottom temp.
mwsh_df$BottomTempAtSurvey <- NA
shf_df$BottomTempAtSurvey <- NA
for (j in 6:44){
  ras.name <- names(r[[j]])
  temp.year <- as.numeric(str_sub(ras.name, 9, 12)) # Year needs to be a number 
  temp.month <- as.numeric(str_sub(ras.name, 14, 14)) # Month needs to be a number 
  if (temp.year %in% shf_df$year & temp.month %in% shf_df$month)
  {
    val_mwsh <- raster::extract(r[[j]], (mwsh_df %>% 
                                           filter(year == temp.year & month == temp.month)))
    val_sh <- raster::extract(r[[j]], (shf_df %>% filter(year == temp.year & 
                                                          month == temp.month)))
    mwsh_df$BottomTempAtSurvey[(mwsh_df$year == temp.year & 
                                  mwsh_df$month == temp.month )] <- val_mwsh[,2]
    shf_df$BottomTempAtSurvey[(shf_df$year == temp.year & 
                                shf_df$month == temp.month )] <- val_sh[,2]
  }
}
